                    ...
                    const void *body, int body_len)
{
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER && log->l_tr_om) {
        ...
        trailer_alignment = log_fcb_hdr_trailer_bytes(fcb->f_align,
                                                      chunk_sz + body_len -
                                                      hdr_alignment);
                         ...
                         struct os_mbuf *om)
{
    ...
    if (hdr->ue_flags & LOG_FLAGS_TRAILER) {
        ...
        if (log->l_tr_om) {
            /* Copy mbuf chain len */
            rc = os_mbuf_copyinto(om, os_mbuf_len(om), pad, trailer_alignment);
            if (rc) {
                return rc;
            }
            ...
            rc = os_mbuf_copyinto(om, os_mbuf_len(om), &trailer_len,
