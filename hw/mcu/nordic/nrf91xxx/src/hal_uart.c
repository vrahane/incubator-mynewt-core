...
hal_uart_start_tx(int port)
{
    ...
    if (port == 0) {
        nrf_uart = NRF_UARTE0_NS;
        u = &uart0;
    } else if (port == 1) {
        nrf_uart = NRF_UARTE1_NS;
        u = &uart1;
    } else if (port == 2) {
        nrf_uart = NRF_UARTE2_NS;
        u = &uart2;
    } else if (port == 3) {
        nrf_uart = NRF_UARTE3_NS;
        u = &uart3;
    } else {
        return;
    }
...
{
    uart_irq_handler(NRF_UARTE0_NS, &uart0);
}

static void
uart1_irq_handler(void)
{
    uart_irq_handler(NRF_UARTE1_NS, &uart1);
}

static void
uart2_irq_handler(void)
{
    uart_irq_handler(NRF_UARTE2_NS, &uart2);
}

static void
uart3_irq_handler(void)
{
    uart_irq_handler(NRF_UARTE3_NS, &uart3);
}
...
{
    struct nrf91_uart_cfg *cfg;
    NRF_UARTE_Type *nrf_uart;

    if (port == 0) {
        nrf_uart = NRF_UARTE0_NS;
        NVIC_SetVector(UARTE0_SPIM0_SPIS0_TWIM0_TWIS0_IRQn, (uint32_t)uart0_irq_handler);
    } else if (port == 1) {
        nrf_uart = NRF_UARTE1_NS;
        NVIC_SetVector(UARTE1_SPIM1_SPIS1_TWIM1_TWIS1_IRQn, (uint32_t)uart1_irq_handler);
    } else if (port == 2) {
        nrf_uart = NRF_UARTE2_NS;
        NVIC_SetVector(UARTE2_SPIM2_SPIS2_TWIM2_TWIS2_IRQn, (uint32_t)uart2_irq_handler);
    } else if (port == 3) {
        nrf_uart = NRF_UARTE3_NS;
        NVIC_SetVector(UARTE3_SPIM3_SPIS3_TWIM3_TWIS3_IRQn, (uint32_t)uart3_irq_handler);
    } else {
        return -1;
    }

    cfg = (struct nrf91_uart_cfg *)arg;
  ...
  enum hal_uart_parity parity, enum hal_uart_flow_ctl flow_ctl)
{
    ...
    if (port == 0) {
        nrf_uart = NRF_UARTE0_NS;
        irqnum = UARTE0_SPIM0_SPIS0_TWIM0_TWIS0_IRQn;
        u = &uart0;
    } else if (port == 1) {
        nrf_uart = NRF_UARTE1_NS;
        irqnum = UARTE1_SPIM1_SPIS1_TWIM1_TWIS1_IRQn;
        u = &uart1;
    } else if (port == 2) {
        nrf_uart = NRF_UARTE2_NS;
        irqnum = UARTE2_SPIM2_SPIS2_TWIM2_TWIS2_IRQn;
        u = &uart2;
    } else if (port == 3) {
        nrf_uart = NRF_UARTE3_NS;
        irqnum = UARTE3_SPIM3_SPIS3_TWIM3_TWIS3_IRQn;
        u = &uart3;
    } else {
        return -1;
    }
