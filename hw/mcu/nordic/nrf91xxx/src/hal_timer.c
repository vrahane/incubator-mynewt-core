...
struct nrf91_hal_timer nrf91_hal_timer4;
#endif

static const struct nrf91_hal_timer *nrf91_hal_timers[NRF91_HAL_TIMER_MAX] = {
#if MYNEWT_VAL(TIMER_0)
    &nrf91_hal_timer0,
#else
    NULL,
...
#define NRF91_HAL_TIMER_RESOLVE(__n, __v)       \
    if ((__n) >= NRF91_HAL_TIMER_MAX) {         \
        rc = EINVAL;                            \
        goto err;                               \
    }                                           \
    (__v) = (struct nrf91_hal_timer *) nrf91_hal_timers[(__n)];            \
    if ((__v) == NULL) {                        \
        rc = EINVAL;                            \
        goto err;                               \
    }
 ...
 */
static void
nrf_timer_set_ocmp(struct nrf91_hal_timer *bsptimer, uint32_t expiry)
{
    int32_t delta_t;
    uint32_t temp;
    uint32_t cntr;
    ...
    if (bsptimer->tmr_rtc) {
        ...
        /*
         * The nRF9160xxx documentation states that COMPARE event is guaranteed
...
static uint32_t
hal_timer_read_bsptimer(struct nrf91_hal_timer *bsptimer)
{
    uint32_t low32;
    uint32_t ctx;
    uint32_t tcntr;
 ...
 */
static void
hal_timer_chk_queue(struct nrf91_hal_timer *bsptimer)
{
    uint32_t tcntr;
    uint32_t ctx;
...
static void
hal_timer_irq_handler(struct nrf91_hal_timer *bsptimer)
{
    uint32_t compare;
    NRF_TIMER_Type *hwtimer;
...
static void
hal_rtc_timer_irq_handler(struct nrf91_hal_timer *bsptimer)
{
    uint32_t overflow;
    uint32_t compare;
    uint32_t tick;
...
#if MYNEWT_VAL(TIMER_0)
void
nrf91_timer0_irq_handler(void)
{
    hal_timer_irq_handler(&nrf91_hal_timer0);
}
#endif

#if MYNEWT_VAL(TIMER_1)
void
nrf91_timer1_irq_handler(void)
{
    hal_timer_irq_handler(&nrf91_hal_timer1);
}
#endif

#if MYNEWT_VAL(TIMER_2)
void
nrf91_timer2_irq_handler(void)
{
    hal_timer_irq_handler(&nrf91_hal_timer2);
}
#endif

#if MYNEWT_VAL(TIMER_3)
void
nrf91_timer3_irq_handler(void)
{
    hal_rtc_timer_irq_handler(&nrf91_hal_timer3);
}
#endif

#if MYNEWT_VAL(TIMER_4)
void
nrf91_timer4_irq_handler(void)
{
    hal_rtc_timer_irq_handler(&nrf91_hal_timer4);
}
#endif
...
hal_timer_init(int timer_num, void *cfg)
{
    ...
    NRF91_HAL_TIMER_RESOLVE(timer_num, bsptimer);
    ...
    case 0:
        irq_num = TIMER0_IRQn;
        hwtimer = NRF_TIMER0_NS;
        irq_isr = nrf91_timer0_irq_handler;
        break;
#endif
#if MYNEWT_VAL(TIMER_1)
    case 1:
        irq_num = TIMER1_IRQn;
        hwtimer = NRF_TIMER1_NS;
        irq_isr = nrf91_timer1_irq_handler;
        break;
#endif
#if MYNEWT_VAL(TIMER_2)
    case 2:
        irq_num = TIMER2_IRQn;
        hwtimer = NRF_TIMER2_NS;
        irq_isr = nrf91_timer2_irq_handler;
        break;
#endif
#if MYNEWT_VAL(TIMER_3)
    case 3:
...
hal_timer_config(int timer_num, uint32_t freq_hz)
{
    ...
    div = NRF91_MAX_TIMER_FREQ / freq_hz;
    ...
    bsptimer->tmr_freq = NRF91_MAX_TIMER_FREQ / (1 << prescaler);
    bsptimer->tmr_enabled = 1;
    ...
    if ((NRF_CLOCK->HFCLKSTAT &
         (NRF_CLOCK_HFCLK_HIGH_ACCURACY | CLOCK_HFCLKSTAT_STATE_Msk)) !=
        (NRF_CLOCK_HFCLK_HIGH_ACCURACY | CLOCK_HFCLKSTAT_STATE_Msk)) {
        ...
        do {
            ...
            while (1) {
                if (i < 1 || ((NRF_CLOCK->EVENTS_HFCLKSTARTED) != 0)) {
                    break;
                }
            }
            i++;
...
hal_timer_start(struct hal_timer *timer, uint32_t ticks)
{
    ...
    bsptimer = (struct nrf91_hal_timer *)timer->bsp_timer;
    if (bsptimer->tmr_rtc) {
...
hal_timer_stop(struct hal_timer *timer)
{
   ...
   bsptimer = (struct nrf91_hal_timer *)timer->bsp_timer;
    ...
    if (timer->link.tqe_prev != NULL) {
        ...
        if (reset_ocmp) {
            if (entry) {
                nrf_timer_set_ocmp((struct nrf91_hal_timer *)entry->bsp_timer,
                                   entry->expiry);
            } else {
                if (bsptimer->tmr_rtc) {
                    nrf_rtc_disable_ocmp((NRF_RTC_Type *)bsptimer->tmr_reg);
