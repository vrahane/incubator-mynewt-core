...
static void hal_flash_readonly_mode_set(void)
{
...
static void hal_flash_write_mode_set(void)
{
#if defined(NVMC_CONFIGNS_WEN_Msk)
...
static void hal_flash_erase_mode_set(void)
{
#if defined(NVMC_CONFIGNS_WEN_Msk)
        ...
        const void *src, uint32_t num_bytes)
{
    ...
    NRF_NVMC->CONFIGNS = NVMC_CONFIG_WEN_Wen; /* Enable erase OP */
    tmp = address & 0x3;
    if (tmp) {
        if (nrf91k_flash_wait_ready()) {
            goto out;
...
nrf91k_flash_erase_sector(const struct hal_flash *dev, uint32_t sector_address)
{
    ...
    NRF_NVMC->CONFIGNS = NVMC_CONFIG_WEN_Een; /* Enable erase OP */
    if (nrf91k_flash_wait_ready()) {
        goto out;
    }
    ...
    rc = 0;
out:
    NRF_NVMC->CONFIGNS = NVMC_CONFIG_WEN_Ren; /* Disable erase OP */
    hal_flash_readonly_mode_set();
    __HAL_ENABLE_INTERRUPTS(sr);
    return rc;
}
