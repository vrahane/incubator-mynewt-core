...
nrf91_os_tick_counter(void)
{
    return OS_TICK_TIMER->COUNTER;
}

static inline void
nrf91_os_tick_set_ocmp(uint32_t ocmp)
{
    int delta;
    uint32_t counter;
    ...
    while (1) {
        ...
        counter = nrf91_os_tick_counter();
        /*
         * From nRF9160 Product specification
         *
...
nrf91_timer_handler(void)
{
    ...
    nrf91_os_tick_set_ocmp(g_hal_os_tick.lastocmp + g_hal_os_tick.ticks_per_ostick);
...
os_tick_init(uint32_t os_ticks_per_sec, int prio)
{
    ...
    NVIC_SetVector(OS_TICK_IRQ, (uint32_t)nrf91_timer_handler);
    NVIC_DisableIRQ(OS_TICK_IRQ);
