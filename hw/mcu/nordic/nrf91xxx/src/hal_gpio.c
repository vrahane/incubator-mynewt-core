...
#else
        if (NRF_GPIOTE1_NS->EVENTS_IN[i] && (NRF_GPIOTE1_NS->INTENSET & (1 << i))) {
...
#if MYNEWT_VAL(MCU_GPIO_USE_PORT_EVENT)
        NRF_GPIOTE1_NS->INTENCLR = GPIOTE_INTENCLR_PORT_Msk;
        NRF_GPIOTE1_NS->EVENTS_PORT = 0;
#endif
    }
}
...
hal_gpio_find_pin(int pin)
{
    ...
    for (i = 0; i < HAL_GPIO_MAX_IRQ; i++) {
        if (hal_gpio_irqs[i].func &&
           (NRF_GPIOTE1_NS->CONFIG[i] & HAL_GPIOTE_PIN_MASK) == pin) {
            return i;
        }
    }
#endif
...
hal_gpio_irq_enable(int pin)
{
    ...
    NRF_GPIOTE1_NS->INTENSET = GPIOTE_INTENSET_PORT_Msk;
#else
    NRF_GPIOTE1_NS->EVENTS_IN[i] = 0;
    NRF_GPIOTE1_NS->INTENSET = 1 << i;
#endif
