...
static const struct nrf91_hal_spi *nrf91_hal_spis[NRF91_HAL_SPI_MAX] = {
#if MYNEWT_VAL(SPI_0_MASTER) || MYNEWT_VAL(SPI_0_SLAVE)
    &nrf91_hal_spi0,
#else
    NULL,
...
#define NRF91_HAL_SPI_RESOLVE(__n, __v)                     \
    if ((__n) >= NRF91_HAL_SPI_MAX) {                       \
        rc = EINVAL;                                        \
        goto err;                                           \
    }                                                       \
    (__v) = (struct nrf91_hal_spi *) nrf91_hal_spis[(__n)]; \
    if ((__v) == NULL) {                                    \
        rc = EINVAL;                                        \
        goto err;                                           \
    }
...
static void
nrf91_irqm_handler(struct nrf91_hal_spi *spi)
{
    NRF_SPIM_Type *spim;
    uint16_t xfr_bytes;
    uint16_t len;
...
#if MYNEWT_VAL(SPI_0_MASTER) || MYNEWT_VAL(SPI_0_SLAVE)
void
nrf91_spi0_irq_handler(void)
{
    os_trace_isr_enter();
    if (nrf91_hal_spi0.spi_type == HAL_SPI_TYPE_MASTER) {
#if MYNEWT_VAL(SPI_0_MASTER)
        nrf91_irqm_handler(&nrf91_hal_spi0);
#endif
    } else {
#if MYNEWT_VAL(SPI_0_SLAVE)
        nrf91_irqs_handler(&nrf91_hal_spi0);
#endif
    }
    os_trace_isr_exit();
}
...
#if MYNEWT_VAL(SPI_1_MASTER)  || MYNEWT_VAL(SPI_1_SLAVE)
void
nrf91_spi1_irq_handler(void)
{
    os_trace_isr_enter();
    if (nrf91_hal_spi1.spi_type == HAL_SPI_TYPE_MASTER) {
#if MYNEWT_VAL(SPI_1_MASTER)
        nrf91_irqm_handler(&nrf91_hal_spi1);
#endif
    } else {
#if MYNEWT_VAL(SPI_1_SLAVE)
        nrf91_irqs_handler(&nrf91_hal_spi1);
#endif
    }
    os_trace_isr_exit();
}
...
#if MYNEWT_VAL(SPI_2_MASTER)  || MYNEWT_VAL(SPI_2_SLAVE)
void
nrf91_spi2_irq_handler(void)
{
    os_trace_isr_enter();
    if (nrf91_hal_spi2.spi_type == HAL_SPI_TYPE_MASTER) {
#if MYNEWT_VAL(SPI_2_MASTER)
        nrf91_irqm_handler(&nrf91_hal_spi2);
#endif
    } else {
#if MYNEWT_VAL(SPI_2_SLAVE)
        nrf91_irqs_handler(&nrf91_hal_spi2);
#endif
    }
    os_trace_isr_exit();
}
...
#if MYNEWT_VAL(SPI_3_MASTER)  || MYNEWT_VAL(SPI_3_SLAVE)
void
nrf91_spi3_irq_handler(void)
{
    os_trace_isr_enter();
    if (nrf91_hal_spi3.spi_type == HAL_SPI_TYPE_MASTER) {
#if MYNEWT_VAL(SPI_3_MASTER)
        nrf91_irqm_handler(&nrf91_hal_spi3);
#endif
    } else {
...
static int
hal_spi_config_master(struct nrf91_hal_spi *spi,
...
static int
hal_spi_config_slave(struct nrf91_hal_spi *spi,
                     struct hal_spi_settings *settings)
{
    int rc;
    uint32_t nrf_config;
...
static int
hal_spi_init_master(struct nrf91_hal_spi *spi,
                    struct nrf91_hal_spi_cfg *cfg,
                    nrf91_spi_irq_handler_t handler)
{
    NRF_SPIM_Type *spim;
    NRF_GPIO_Type *port;
    uint32_t pin;
...
static int
hal_spi_init_slave(struct nrf91_hal_spi *spi,
                   struct nrf91_hal_spi_cfg *cfg,
                   nrf91_spi_irq_handler_t handler)
{
    NRF_SPIS_Type *spis;
...
hal_spi_init(int spi_num, void *cfg, uint8_t spi_type)
{
    ...
    nrf91_spi_irq_handler_t irq_handler;
...
#if MYNEWT_VAL(SPI_3_MASTER)  || MYNEWT_VAL(SPI_2_SLAVE)
    ...
    if (spi_type == HAL_SPI_TYPE_MASTER) {
        ...
        rc = hal_spi_init_slave(spi, (struct nrf91_hal_spi_cfg *)cfg,
                                irq_handler);
    }
...
{
    struct nrf91_hal_spi_cfg hal_cfg;
